...
import 'package:abyadpos_tab/core/pdf_export/lib/export_instance.dart';
...
import 'package:abyadpos_tab/core/pdf_export/lib/options/font_data.dart';
...
class ExportDelegate {
  ...
  final Map<String, ExportFrameState> _registeredFrames = {};

  /// Registers a new [ExportFrameState] to the [ExportDelegate].
  void registerFrame(ExportFrameState frameState) {
    _registeredFrames[frameState.widget.frameId] = frameState;
  }

  /// Returns the [ExportFrameState] with the given [frameId].
  /// Throws an [Exception] if no [ExportFrameState] is found.
  ExportFrameState getFrame(String frameId) {
    final ExportFrameState? frameState = _registeredFrames[frameId];
  ...
  Future<pw.Document> exportToPdfDocument(String frameId,
    ...
    final ExportFrameState frameState = getFrame(frameId);

    if (overrideOptions != null) {
      ExportDelegate delegate = copyWith(options: overrideOptions);
      return await delegate._exportDocument(
          frameState.exportWidget, frameState.exportContext!);
    }

    return await _exportDocument(
        frameState.exportWidget, frameState.exportContext!);
  }
